...
class Model(nn.Module):
    ...
    def __init__(self, n_channels=3, latent_dim=10, bilinear=True):
        ...
        # discriminator
        self.discriminator1 = Up(1024, 256)
        self.discriminator2 = Up(512, 128)
        self.discriminator3 = Up(256, 64)
        self.discriminator4 = Up(128, 1)
    ...
    #     decode_input=nn.Linear(self.latent_dim, 512)(input).to(torch.device("cuda"))
    #     return decode_input.view(-1, 512, 1, 1)
    ...
    def forward(self, x):
        ...
        for index, discriminator_input in enumerate([z_fake, z_real]):
            # discriminator
            # discriminator_input = self.decode_input(z)
            discriminator1_result = self.discriminator1(discriminator_input, x3_3)
            discriminator2_result = self.discriminator2(discriminator1_result, x2_3)
            discriminator3_result = self.discriminator3(discriminator2_result, x1_3)
            discriminator4_result = self.discriminator4(discriminator3_result, x0_2)
            # discriminator = self.discriminator1()
            discriminator_z_result[index] = discriminator4_result
        
        return out, discriminator_z_result[0], discriminator_z_result[1]    # out, z_fake_result, z_real_result

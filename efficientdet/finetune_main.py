...
def main(_):
  ...
  if FLAGS.mode == 'train':
    ...
    try:
      ...
      current_epoch = 0
    # reset step and current_epoch information for fine-tuning. 
    step = 0 
    current_epoch = 0
    
    def run_train_and_eval(e):
      print('\n   =====> Starting training, epoch: %d.' % e)
      # print('\n num_ex_per_epoch:%d'%(FLAGS.num_examples_per_epoch))
      # print('\n train_batch_size:%d'%(FLAGS.train_batch_size))
      # train_est.train(
      #     input_fn=train_input_fn,
      #     max_steps=e * FLAGS.num_examples_per_epoch // FLAGS.train_batch_size)
      train_est.train(
          input_fn=train_input_fn,
          steps=FLAGS.num_examples_per_epoch // FLAGS.train_batch_size)
      # import sys
      # sys.exit()

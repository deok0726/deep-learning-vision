...
class EvaluationMetric():
  ...
  def reset_states(self):
    ...
    self.metric_values_re = None

  def evaluate(self, select):
    ...
    if self.testdev_dir:
      ...
      coco_metrics_re = coco_eval.stats
      # precision = coco_eval.eval['precision'][:, :, :, 0, -1]
      # recall = coco_eval.eval['recall'][:, :, :, -1]
     
      if self.label_map and select == 'precision':
        print('precision metric called')
        ...
        print("="*100)
        print('precision mean adding class recall with 0.01 difference')
        # print(precision[:,:,0])
        print(np.mean(precision, axis=2))
        recall_x = np.linspace(0, 1, num=101)

        # overall average of all classes
        # precision_y = np.mean(precision, axis=(0,2))
        # plt.plot(recall_x, precision_y)

        # overall classes with indivial IoU's 0.5, 0.75, 0.95
        precision_y = np.mean(precision, axis=2)
        plt.plot(recall_x, precision_y[0,:], color='red') 
        plt.plot(recall_x, precision_y[5,:], color='green')
        plt.plot(recall_x, precision_y[9,:], color='blue')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title('Overall PR curve by IoU')
        # plt.legend(['average of all classes and all IoU threshold'])
        plt.legend(['IoU 0.5', 'IoU 0.75', 'IoU 0.95'])
        # plt.show()

        plt.savefig('/root/pr_overall_by_IoU.png')

        print("="*100)
        plt.figure()
        legend_list = []
        for c in range(precision.shape[-1]):  # iterate over all classes
          precision_cls = precision[0,:,c]
          plt.plot(recall_x, precision_cls)
          legend_list.append(self.label_map[c + 1])
          
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.legend(legend_list)
        plt.title('PR curve for each class at IoU threshold 0.5')
        plt.savefig('/root/pr_each_class_IoU_0_5.png')
        
        # print(sum(precision[0,:,0]) / 101)
        ...
        # print("DEBUG!!:precision", precision.shape, precision)
        for c in range(precision.shape[-1]):  # iterate over all classes
          precision_c = precision[:, :, c]
          # Only consider values if > -1.
          precision_c = precision_c[precision_c > -1]
          # print("DEBUG!!:per class precision", precision_c)
          ap_c = np.mean(precision_c) if precision_c.size else -1.
          ap_perclass[c] = ap_c
        
        ap_50_perclass = [0] * max(precision.shape[-1], len(self.label_map))
        for c in range(precision.shape[-1]):  # iterate over all classes
          precision_50_c = precision[0, :, c]
          precision_50_c = precision_50_c[precision_50_c > -1]
          # print("DEBUG!!:per class 50 precision", precision_50_c.shape , precision_50_c)
          ap_50c = np.mean(precision_50_c) if precision_50_c.size else -1.
          ap_50_perclass[c] = ap_50c
          # print(ap_50c)
        
        coco_metrics = np.concatenate((coco_metrics, ap_perclass))
        coco_metrics = np.concatenate((coco_metrics, ap_50_perclass))
        # Return the concat normal and per-class AP.
        # return np.array(coco_metrics, dtype=np.float32), np.array(coco_metrics_re, dtype=np.float32)
        return np.array(coco_metrics, dtype=np.float32)

        
        ###############################################################################

      if self.label_map and select == 'recall':
        print('recall metric called')
        
        recall = coco_eval.eval['recall'][:, :, :, -1]
        re_perclass = [0] * max(recall.shape[-1], len(self.label_map))
        for c in range(recall.shape[-1]):  # iterate over all classes
          recall_c = recall[:, :, c]
          recall_c = recall_c[recall_c > -1]
          re_c = np.mean(recall_c) if recall_c.size else -1.
          re_perclass[c] = re_c
        
        re_50_perclass = [0] * max(recall.shape[-1], len(self.label_map))
        for c in range(recall.shape[-1]):  # iterate over all classes
          recall_50_c = recall[0, :, c]
          recall_50_c = recall_50_c[recall_50_c > -1]
          re_50c = np.mean(recall_50_c) if recall_50_c.size else -1.
          re_50_perclass[c] = re_50c

        coco_metrics_re = np.concatenate((coco_metrics_re, re_perclass))
        coco_metrics_re = np.concatenate((coco_metrics_re, re_50_perclass))
        return np.array(coco_metrics_re, dtype=np.float32)

  def result(self):
    """Return the metric values (and compute it if needed)."""
    print('precision result called')
    if self.metric_values is None:
      self.metric_values = self.evaluate('precision')
    return self.metric_values

  def result_re(self):
    print('recall result called')
    if self.metric_values_re is None:
      self.metric_values_re = self.evaluate('recall')
    return self.metric_values_re
    ...
    with tf.name_scope('coco_metric'):
      if self.testdev_dir:
        ...
        if self.label_map:
          ...
          for i, cid in enumerate(sorted(label_map.keys())):
            name = 'AP(IoU_50)_per_class/%s' % label_map[cid]
            metrics_dict[name] = (metrics[i + len(self.metric_names) + len(label_map.keys())],
                                  update_op)

          # for i, cid in enumerate(sorted(label_map.keys())):
          #   name = 'RC(IoU_50)_per_class/%s' % label_map[cid]
          #   metrics_dict[name] = (metrics[i + len(self.metric_names) + len(label_map.keys())],
          #                         update_op)

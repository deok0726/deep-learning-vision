...
class ConvolutionAutoEncoder(AbstractModel):
    ...
    def __init__(
        ...
        if encoder_and_opt is None:
            layer_list = [
                nn.Conv2d(input_channel, 32, kernel_size=4,stride=2,padding=1), #conv1
                nn.LeakyReLU(0.2),
                nn.Conv2d(32, 32, kernel_size=4,stride=2,padding=1), #conv2
                nn.LeakyReLU(0.2),
                nn.Conv2d(32, 32, kernel_size=4,stride=2,padding=1), #conv3
                nn.LeakyReLU(0.2),
                nn.Conv2d(32, 32, kernel_size=3,stride=1,padding=1), #conv4
                nn.LeakyReLU(0.2),
                nn.Conv2d(32, 64, kernel_size=4,stride=2,padding=1), #conv5
                nn.LeakyReLU(0.2),
                nn.Conv2d(64, 64, kernel_size=3,stride=1,padding=1), #conv6
                nn.LeakyReLU(0.2),
                nn.Conv2d(64, 128, kernel_size=4,stride=2,padding=1), #conv7
                nn.LeakyReLU(0.2),
                nn.Conv2d(128, 64, kernel_size=3,stride=1,padding=1), #conv8
                nn.LeakyReLU(0.2),
                nn.Conv2d(64, 32, kernel_size=3,stride=1,padding=1), #conv9
                nn.LeakyReLU(0.2),
                nn.Conv2d(32, btl_size, kernel_size=8,stride=1,padding=0) #conv10
            ]
        ...
        if decoder_and_opt is None:
            layer_list = [
                nn.ConvTranspose2d(btl_size, 32, kernel_size=8,stride=1,padding=0),
                nn.LeakyReLU(0.2),

...
class ConvolutionAutoEncoder(AbstractModel):
    ...
    def __init__(
        ...
        if encoder_and_opt is None:
            layer_list = [
                nn.Conv2d(input_channel, 16, kernel_size=1,stride=2, padding=1), #conv1
                nn.BatchNorm2d(16),
                nn.ReLU(),
        ...
        if decoder_and_opt is None:
            layer_list = [
                nn.ConvTranspose2d(btl_size, 32, kernel_size=3,stride=2, padding=1, output_padding=1),
                nn.BatchNorm2d(32),
                nn.ReLU(),
                nn.ConvTranspose2d(32, 16, kernel_size=2,stride=2, padding=1, output_padding=1),
                nn.BatchNorm2d(16),
                nn.ReLU(),
                nn.ConvTranspose2d(16, input_channel, kernel_size=2,stride=2, padding=1),
            ]

...
class ConvolutionAutoEncoder(AbstractModel):
    ...
    def __init__(
        ...
        if encoder_and_opt is None:
            layer_list = [
                nn.Conv2d(input_channel, 64, kernel_size=1,stride=2), #conv1
                nn.BatchNorm2d(64),
                nn.ReLU(),
                nn.Conv2d(64, 128, kernel_size=3,stride=2, padding=1), #conv3
                nn.BatchNorm2d(128),
                nn.ReLU(),
                nn.Conv2d(128, 128, kernel_size=3,stride=1, padding=1), #conv3
                nn.BatchNorm2d(128),
                nn.ReLU(),
                nn.Conv2d(128, 256, kernel_size=3,stride=2, padding=1), #conv8
                nn.BatchNorm2d(256),
                nn.ReLU(),
            ]
        ...
        if decoder_and_opt is None:
            layer_list = [
                ...
                nn.BatchNorm2d(64),
                nn.ReLU(),
                nn.ConvTranspose2d(64, input_channel, kernel_size=3,stride=2, padding=1, output_padding=1),
            ]

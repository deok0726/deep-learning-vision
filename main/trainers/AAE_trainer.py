...
class AAE_trainer(Trainer):
    ...
    def _train_step(self, batch_data, batch_label, optimizer_idx = 0):
        ...
        # discriminator loss
        # EPS = 1e-15
        # discriminator_loss = torch.log(discriminator_z_real + EPS) + torch.log(1 - discriminator_z_fake + EPS)
        # self.losses_per_batch['discriminator'] = discriminator_loss
        # self.train_losses_per_epoch['discriminator'].update(discriminator_loss.mean().item())
        #####################################################

...
# def rgba2rgb(images):
#     rgba_image = Image.fromarray(images.astype('uint8'))
...
def get_x(obj, mode='train'):
    ...
    if mode == 'test':
        ...
        # if images1.shape[-1] == 4:
        #     print('Change rgba to rgb')
        #     images1 = rgba2rgb(images1)
        #     print(images1.shape)
        ...
        # for f in test_files:
        #     Path(f).rename(os.path.join(f'/hd/daejoo/{obj}/test/good/', os.path.basename(f)))
        
        # valid_files = fpaths[num_test:num_test+num_valid] 
        # print('Num of valid_files :', len(valid_files))
        # for f in valid_files:
        #     Path(f).rename(os.path.join(f'/hd/daejoo/{obj}/valid/good/', os.path.basename(f)))
